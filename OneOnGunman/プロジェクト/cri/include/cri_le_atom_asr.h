 ...
 * 失敗した場合、CRIATOMEXASR_RACK_ILLEGAL_IDが返されます。
 */
 ...
 * \par 説明:
 * バイノーライザーの有効を指定します。<br>
 * \par 備考:
 * バイノーライザーは sound_renderer_type が CRIATOM_SOUND_RENDERER_SPATIAL であるASRラックでのみ使用可能です。
...
/*JP
 * \brief バイノーライザーの有効化状態の取得
 * \ingroup ATOMEXLIB_ASR
 * \return						バイノーライザーの有効か？（CRI_TRUE:有効, CRI_FALSE:無効）
 * \par 説明:
 * バイノーライザーの有効化状態を取得します。<br>
 */
...
/*JP
 * \brief スペシャライザーインタフェースの不完全型
 * \par 説明:
 * スペシャライザーフェース登録関数の引数型です。
 * \sa criAtomExAsr_RegisterSpatializerInterface
 */
typedef const struct CriSpatializerInterfaceTag* CriAtomExAsrSpatializerInterfacePtr;

/*JP
 * \brief スペシャライザー初期化コンフィグの不完全型
 * \par 説明:
 * スペシャライザーフェース登録関数の引数型です。
 * \sa criAtomExAsr_RegisterSpatializerInterface
 */
typedef struct CriSpatializerInitializeConfigTag* CriAtomExAsrSpatializerInitializeConfigPtr;

/*JP
 * \brief ユーザ定義スペシャライザーインターフェースの登録
 * \ingroup ATOMEXLIB_ASR
 * \param[in]	spatializer_interface	ユーザ定義スペシャライザーのバージョン情報付きインターフェース
 * \param[in]	init_config				ユーザ定義スペシャライザーの初期化コンフィグ
 * \return						登録に成功したか？（CRI_TRUE:登録に成功した, CRI_FALSE:登録に失敗した）
 * \par 説明:
 * ユーザ定義スペシャライザーインターフェースを登録します。<br>
 * sound_render_type に CRI_ATOM_SOUND_RENDERER_SPATIAL を含むレンダラタイプを指定したASRラックは、
 * 内部でスペシャライザーが作成されます。<br>
 * スペシャライザー作成の際、そのスペシャライザーのタイプのインタフェースが登録されていると、インタフェースを使用して
 * スペシャライザーを作成します。
 * 一つのスペシャライザーインタフェースが複数のタイプに対応している場合、それぞれのタイプに登録する必要があります。
 * \attention
 * 本関数は、登録したタイプのスペシャライザーを含むASRラックが作成される前に実行してください。
 */
CriBool CRIAPI criAtomExAsr_RegisterSpatializerInterface(CriSint32 spatializer_type, CriAtomExAsrSpatializerInterfacePtr spatializer_interface, CriAtomExAsrSpatializerInitializeConfigPtr init_config);
